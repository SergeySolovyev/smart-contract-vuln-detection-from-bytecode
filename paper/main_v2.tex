...
without decoding it to bytes. Re-attaching the deployed bytecode from the
upstream corpus and retraining the best configuration on properly decoded
opcodes gives validation macro-$F_1$ \vDecodedVal{} against \vLegacyVal{} for
the same configuration on the legacy tokens and \vDecXgbVal{} for XGBoost on
the same split; the classical multi-label fit takes \vXgbFitS~s on
\vXgbCpus{} CPU cores against \vDecodedMin~min of GPU time. We also measure what the earlier, text-deduplicated split
...
found to be defective (Sec.~\ref{sec:methods}), together with a
validation-level retraining of the best configuration on properly decoded
opcodes (Sec.~\ref{sec:ml-v2}).
...
encoding of the bytecode rather than decoded opcodes, so the test-split
result is a result about that representation.
Retraining the best configuration on properly decoded opcodes, scored on
the validation split, leaves gradient boosting ahead by
\vDecXgbGapAbs~p.p.; a test-level confirmation needs a partition that
excludes the present test split.
...
metrics are checked against saved records rather than recomputed. A second
executed notebook re-attaches the deployed bytecode from the upstream corpus
and runs the decoded-opcode comparison and the classical timing of
Sec.~\ref{sec:ml-v2}; it opens no test row. Exact refitting of the reference run is not claimed because fit-time
...
\item Decoded-opcode run and classical timing:
\url{https://github.com/SergeySolovyev/smart-contract-vuln-detection-from-bytecode/tree/main/results/decoded_c2}
\end{itemize}
