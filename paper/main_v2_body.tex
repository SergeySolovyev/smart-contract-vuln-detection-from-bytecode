...
\textbf{(a)~Duplicate removal:} performed twice, on metadata-stripped
runtime bytecode and on the \vNFeat-dimensional feature vector, before the
split; the no-overlap invariant is asserted at build time.
...
family whose members differ in a handful of features survives it. This residue is smaller than the exact-twin effect we measured and removed, and we defer address-grouped re-splitting via
the Etherscan proxy-detection API, together with a temporal train/test cut,
...
wrapped in \texttt{sklearn.MultiOutputClassifier} predict the eight
SWC-derived classes (access-control, arithmetic, bad-randomness,
...
\paragraph{Tier-1 operating envelope.}
The point of a false-negative rate of \vBXGBFnr{} is what it implies for the
funnel. Screening every deployment and forwarding the most suspicious
${\sim}1\%$ to Slither ($\sim\!1$~s/contract) and the critical remainder to
symbolic search (minutes/contract) reduces the deep-analysis workload by
roughly two orders of magnitude, at the cost of the missed positives that
\vBXGBFnr{} quantifies. 
...
\vDlNConfigs{} Conv-Transformer configurations. Classes are ordered as in the SWC taxonomy;
...
The training time of the classical multi-label baseline was not recorded at
all. We therefore state the compute comparison as an order of magnitude and
decline to give a ratio: a mean over the \vDlTimeN{} runs that happen to
carry a timestamp would read as a mean over all \vDlTimeOf. The earlier version reported exactly such a mean, and a $30\times$ ratio with it. What the surviving timings do establish is that the comparator is not losing for want of training: it loses in every configuration, under every loss function, at roughly an order of magnitude more compute.
...
models and fit-time environment were not saved (Sec.~\ref{sec:binary-v2}).
To close that gap from the other side, the author's original experiment
notebook was restructured into a single document with two modes, a local
teaching mode and a full mode, and the full mode was run to completion on a
Kaggle \vRunGpu{} (kernel version \vRunVer). The executed notebook has
\vRunCells{} cells; all \vRunCode{} code cells carry execution counts and
none carries an error output. It is released in the archive with every
artifact this section cites.

\paragraph{Protocol.}
The run trains \vRunStages{} model stages, seven classical and \vRunDl{}
deep, on the released training split, ranks them on the validation split,
freezes a final set of three before any test access, and then reads the test
split once. The frozen set is fixed by the method rather than by the ranking:
the multi-label logistic reference, the validation-selected deep
configuration, and the binary logistic reference. A threshold-sensitivity
diagnostic on the four loss variants runs after the freeze and outside
selection. Quota-bounded sessions were resumed from identity-checked
checkpoints, so the deep models retain their epoch counts (\vRunEpochsLo{}
to \vRunEpochsHi) but only the last-trained stage retains a per-epoch curve.
The training loop returns the last executed epoch's weights rather than
restoring the best one.

\begin{table}[t]
\centering
\caption{Reference pipeline against the end-to-end re-execution. The
re-execution scored only its frozen final set on the test split; the lower
block compares its validation scores, where every model was scored, with the
reference test scores. Binary LogReg reproduces to three decimals;
multi-label LogReg differs because the notebook standardises inputs for that
reference and the script did not.}
\label{tab:rerun-v2}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lcc@{}}
\toprule
 & \textbf{Reference} & \textbf{Re-execution} \\
\midrule
...
Best deep, macro-$F_1$ (\vRunDlBestName{} in both) & \vDlBest & \vRunDlBestTest \\
\midrule
\multicolumn{3}{@{}l}{\emph{Reference test vs.\ re-execution validation}} \\
Best deep, macro-$F_1$ & \vDlBest & \vRunDlBestVal \\
XGBoost macro-$F_1$ & \vMlXgb & \vRunMlXgbVal \\
XGBoost minus best deep, p.p. & \vDlGap & \vRunGapVal \\
...
deterministic solver and the same released split give the same $F_1$, MCC
and PR-AUC. The re-execution again selects \vRunDlBestName{} as the
strongest deep configuration, now on validation (\vRunDlBestVal; worst
\vRunDlWorstName{} at \vRunDlWorstVal, mean \vRunDlMeanVal), and XGBoost
again beats every deep configuration, by \vRunGapVal~p.p.\ on validation
against \vDlGap~p.p.\ on the reference test split. The ordering of
Sec.~\ref{sec:ml-v2} is therefore not an artefact of the original scripts.

\paragraph{What differs, and why.}
The multi-label logistic reference rises from \vMlLr{} to \vRunMlLr{}
because the notebook standardises its inputs and the historical script did
not; the difference measures standardisation, not reproduction. The selected
deep configuration reaches \vRunDlBestTest{} on the test split against
\vDlBest{} in the reference. Three protocol differences are candidates:
last-epoch rather than best-epoch weights, a resumed training chain, and a
single seed in both runs. We do not apportion the gap among them; we report
it as the variation a reader should expect between two executions of this
experiment. Only three models were scored on the test split, by design, so
the per-class sweep of Sec.~\ref{sec:ml-v2} is not repeated there. Tuning
the four loss-variant thresholds on validation moved their validation
macro-$F_1$ by \vRunBFiveLo{} to \vRunBFiveHi; because those thresholds
were assessed on the split that chose them, this is a sensitivity figure and
no part of selection.

\paragraph{What the re-execution does not establish.}
It reads the same test split that Sec.~\ref{sec:ml-v2} read. Two reads of
one partition by two pipelines are still one partition, and any later change
motivated by either result cannot be confirmed on it. The
corrected-representation experiment that the caveat in
Sec.~\ref{sec:methods} calls for is specified in the archive as a separate
protocol, with a family-grouped re-split, a sealed confirmatory partition
that excludes every contract of the present test split, and pre-registered
decision rules; it has not been run.

\section{Discussion and Future Work}\label{sec:disc}

\paragraph{Why deep learning did not help here, and what that does not show.}
The \vNFeat{} engineered features --- opcode counts, control-flow
statistics, gas aggregates, SWC-aligned risk scores --- form a
mid-dimensional tabular representation, exactly the regime in which
tree-based ensembles outperform deep networks~\cite{grinsztajn2022,
shwartz2022}. The discriminative signal is statistical, and the
Conv-Transformer must re-derive from its token sequences what XGBoost reads
directly off a column. Our contribution is not the mechanism, which is
known, but its demonstration on EVM bytecode features under a stated
protocol. The practical reading is narrow and worth
stating as such: do not reach for a Conv-Transformer on features like these
unless you are also changing the representation to one that leaves the
tabular regime --- a raw control-flow graph, or a cross-contract graph.

The representation caveat of Sec.~\ref{sec:methods} weakens that
attribution without reversing the measurement. The comparator saw a
hexadecimal-character encoding of the bytecode: invertible, so no
information was destroyed, but with every instruction boundary and every
\texttt{PUSH} operand displaced. A network that must first learn to pair
hexadecimal digits into bytes is handicapped for a reason that has nothing
to do with the tabular regime. The finding stands as measured --- on this
corpus, under this protocol, with this representation, gradient boosting on
\vNFeat{} features wins every class --- but the explanation is now one of
two candidates, and this design cannot separate them.
