...
error. Every deep-learning number in this paper describes the legacy
sequence pipeline, not canonical EVM opcode modelling.
...
\paragraph{Why the comparator lost is not established.}
...
networks~\cite{grinsztajn2022,shwartz2022}. That is one candidate
explanation for the gap, and this design cannot separate it from the
representation defect of Sec.~\ref{sec:methods}. What is measured is
narrower: on this corpus, under this protocol, with this representation,
gradient boosting on \vNFeat{} features scores higher on every class.
...
comparison, the input needs correction: decode the
...
canonical EVM opcode sequences. The audited numeric extractor decodes bytes
first; this is not an all-row semantic validation of its features.
...
the classical multi-label fit was not timed and inference cost was not
measured under the present pipeline, so no compute or throughput
comparison is drawn.
