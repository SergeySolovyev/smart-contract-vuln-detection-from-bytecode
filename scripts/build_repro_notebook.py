...
import os as _os
# Same convention as the other repo scripts: default to the results/ tree
# shipped in this checkout; point PAPER_V2_DIR at a full working tree to
# regenerate from scratch.
V2 = Path(_os.environ.get("PAPER_V2_DIR")
...
_fc = V2 / "feature_columns.json"
if not _fc.exists():
    _fc = Path(__file__).resolve().parent.parent / "data" / "feature_columns.json"
feats = json.load(open(_fc, encoding="utf-8"))
...
**Ablation coverage.** {len(PUBLISHED['dl'])} of 10 planned deep
configurations completed; `C3_pure_cnn` and `C4_pure_transformer` exhausted
the GPU allocation and are excluded rather than estimated.
"""))
